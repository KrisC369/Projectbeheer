...
public abstract class CreateEventBaseImpl extends  DataObject
	
{
	public static Logger logger = Logger.getLogger("LIBs0199831.CreateEvent");
	
	public CreateEventBaseImpl () {
		super();
		addListeners();
	}
	...
	*/
	...
	static {
		q= new VSQueryDefinition( "CreateEvent" );	
		q.setPackageName(deployedFromRepository);
		
		VSMetaColumn c = null;
		VSMetaTable t = new VSMetaTable("CreateEvent");
		//{{META_QUERY_COLUMN_CTOR
		c = new VSMetaColumn("PKCreateEvent", DataConst.BIGINT);	
		...
		t.addUniqueKeyColumn( "CreateEvent Unique Key", "PKCreateEvent" );
	...
	//END_META_QUERY_COLUMN_CTOR}}
		q.addTable( t );
		q.populateQCArray();
		VSQueryDefinition qTemp = (VSQueryDefinition)getMetaQuery( "CreateEvent",deployedFromRepository ); 
		if ( qTemp == null ) {			
