...
public abstract class ReturnEventBaseImpl extends  DataObject
	
{
	public static Logger logger = Logger.getLogger("LIBs0199831.ReturnEvent");
	
	public ReturnEventBaseImpl () {
		super();
		addListeners();
	}
	...
	*/
	...
	static {
		q= new VSQueryDefinition( "ReturnEvent" );	
		q.setPackageName(deployedFromRepository);
		
		VSMetaColumn c = null;
		VSMetaTable t = new VSMetaTable("ReturnEvent");
		//{{META_QUERY_COLUMN_CTOR
		c = new VSMetaColumn("PKReturnEvent", DataConst.BIGINT);	
	...
	//END_META_QUERY_COLUMN_CTOR}}
		q.addTable( t );
		q.populateQCArray();
		VSQueryDefinition qTemp = (VSQueryDefinition)getMetaQuery( "ReturnEvent",deployedFromRepository ); 
		if ( qTemp == null ) {			
...
/**	  
* <br>
* Constructor for the class ReturnEventBaseImpl.
...
*/
public	ReturnEventBaseImpl(Session session, boolean makeDefaults)
{
	super(session, ReturnEventBaseImpl.getMetaQuery(), makeDefaults);
	addListeners();
}
...
public static void initCache() {
	...
	AppObjectImpl appObject = AppObjectImpl.getBusinessObject( deployedFromRepository, "ReturnEvent", (Session)ServerEnvironment.getServer().getInternalSession());
	...
	throws ServerException
{
	if ( aSession.getSecurityCheck() ) {
		try {
		if (!aSession.getMyPrivilegeToObjectName(DataConst.AppObjectPrivilegeImpl_READ, "ReturnEvent", DataConst.AppObjectTypeCodeImpl_BUSINESS_OBJECT)) {
			throw new ServerException("", VSErrors.VSMSG_SecurityNoReadAccess, "business", "ReturnEvent","", (String)null);
		}
		}
	...
	if (aSession.isTransactionInProgress()) {
		Enumeration e= aSession.getTransactionInfo().getObjects(ReturnEventBaseImpl.getMetaQuery(), xdac, searchReq);
		...
		if ( cacheSize!=0 && e.hasMoreElements()) {
			...
			CacheAgent.getCacheAgent().setObjects( ReturnEventBaseImpl.getMetaQuery(), boList, expires );
		...
		ResultSet rs = new VSORBResultSetImpl( ReturnEventBaseImpl.getMetaQuery(), searchReq, null, -1 ,DataConst.NONE_LOB, xdac, con );
...
public static void raiseBeforeQueryEvent( SearchRequest searchReq, Session aSession )	{
	...
	if ( VSTrace.IS_ON ) {
		tr = VSTrace.get(); 
		tr_id = tr.beg(logger);
		tr.set(VST_CATEGORY,VST_VLS_EVENT_QUERY).set(VST_ACTION_NAME,"ReturnEvent.BeforeQuery");
	}

	beforeQuery( "ReturnEvent", searchReq, null , aSession );
...
public static void raiseAfterQueryEvent( ResultSet rs)	{
	...
	if ( VSTrace.IS_ON ) {
		tr = VSTrace.get(); 
		tr_id = tr.beg(logger);
		tr.set(VST_CATEGORY,VST_VLS_EVENT_QUERY).set(VST_ACTION_NAME,"ReturnEvent.AfterQuery");
	}
	...
	throws ServerException
{
	...
	//**************************** cache support
	if ( cacheSize != 0 )    {
		anObject = CacheAgent.getCacheAgent().getObject( ReturnEventBaseImpl.getMetaQuery(),key.parameterList, aSession);
	...
	if (aSession.isTransactionInProgress()) {
		anObject = aSession.getTransactionInfo().getObjectByKey(ReturnEventBaseImpl.getMetaQuery(),key);
	} else {		
		anObject = aSession.getObjectByKey(ReturnEventBaseImpl.getMetaQuery(),key);
	}
	
	//**************************** cache support
	if ( cacheSize != 0 )
		 CacheAgent.getCacheAgent().addObject( ReturnEventBaseImpl.getMetaQuery(),anObject, expires );
...
protected void tableConditionActions() {
	...
	if ( VSTrace.IS_ON ) {
		tr = VSTrace.get(); 
		tr_id = tr.beg(logger);
		tr.set(VST_CATEGORY,"VST_RULE_CONDITIONACTION").set(VST_ACTION_NAME, this.getClass().getName() + "_returnCopy");
	}
	try {
	
		if (  ( isInserted() == true )  )
		{
			getLoan( ).getCopy( ).setCopyState(5);
		}
	} finally {
		if ( tr != null ) tr.end( tr_id );
	}
	...
	if ( VSTrace.IS_ON ) {
		tr = VSTrace.get(); 
		tr_id = tr.beg(logger);
		tr.set(VST_CATEGORY,"VST_RULE_CONDITIONACTION").set(VST_ACTION_NAME, this.getClass().getName() + "_returnLoan");
	}
	try {
	
		if (  ( isInserted() == true )  )
		{
			getLoan( ).setLoanState(2);
		}
	} finally {
		if ( tr != null ) tr.end( tr_id );
	}
...
protected void insert() throws ServerException
{
	...
	if ( xdac instanceof XDASQLConnector ) {
		if ( ( counter = ((XDASQLConnector)xdac).getCounter(ReturnEventBaseImpl.getMetaQuery().getBaseTable("ReturnEvent"), 
		   session.getTransactionInfo().getConnection( xdac), rowData,"PKReturnEvent" ) )!= null )	
		   this.setPKReturnEvent( ((Long)counter).longValue());
	}
	else {
		if ( ( counter = xdac.getCounter(ReturnEventBaseImpl.getMetaQuery().getBaseTable("ReturnEvent"), 
...
protected void update() throws ServerException
{
	...
	{		
		raiseException("Attribute 'FKLoan' in Object ReturnEvent is not Alterable. Error Column: <ReturnEvent>.<FKLoan>");
	}
	if ( getGlobalNestLevel() == 1 && isAltered("PKReturnEvent") && !isUpdatedAfterInsert() )
	{		
		raiseException("Attribute 'PKReturnEvent' in Object ReturnEvent is not Alterable. Error Column: <ReturnEvent>.<PKReturnEvent>");
	}
...
protected void setFormulaValuesWithoutException()
{
	...
	try {
		if ( VSTrace.IS_ON ) {
			tr = VSTrace.get(); 
			tr_id = tr.beg(logger);
			tr.set(VST_CATEGORY,VST_RULE).set(VST_ACTION_NAME,"tableConstraintCheck").set(VST_OBJECT_NAME,"ReturnEvent_Copy 1");
		}

		if ( ! (getLoan( ).getCopy( ).getCopyState( ) == 3 || getLoan( ).getCopy( ).getCopyState( ) == 4) ) {
			raiseException("Invalid Copy state. Error Column: <ReturnEvent>.<FKLoan>");
		}
	} catch (Exception ex) {
	 	String dependents[] = { };
	...
	try {
		if ( VSTrace.IS_ON ) {
			tr = VSTrace.get(); 
			tr_id = tr.beg(logger);
			tr.set(VST_CATEGORY,VST_RULE).set(VST_ACTION_NAME,"tableConstraintCheck").set(VST_OBJECT_NAME,"ReturnEvent_Member 1");
		}

		if ( ! (getLoan( ).getMember( ).getMemberState( ) == 1) ) {
			raiseException("Invalid member state Error Column: <ReturnEvent>.<FKLoan>");
		}
	} catch (Exception ex) {
	 	String dependents[] = { };
	...
	try {
		if ( VSTrace.IS_ON ) {
			tr = VSTrace.get(); 
			tr_id = tr.beg(logger);
			tr.set(VST_CATEGORY,VST_RULE).set(VST_ACTION_NAME,"tableConstraintCheck").set(VST_OBJECT_NAME,"ReturnEvent_Loan 1");
		}

		if ( ! (getLoan( ).getLoanState( ) == 1) ) {
			raiseException("Loan is not active Error Column: <ReturnEvent>.<FKLoan>");
		}
	} catch (Exception ex) {
	 	String dependents[] = { };
...
public void columnValidationCheck() 
{
	...
	if ( VSTrace.IS_ON ) {
		tr = VSTrace.get(); 
		tr_id = tr.beg(logger);
		tr.set(VST_CATEGORY,VST_RULE).set(VST_ACTION_NAME,"parentCheckFor_Loan").set(VST_OBJECT_NAME,"ReturnEvent");
	}
	...
	((!isNull("FKLoan")) ))
	{
		...
		if ( parent == null )
		{
			raiseException("Loan not found for ReturnEvent. Error Column: <ReturnEvent>.<FKLoan>");
		}
		else
		{
			// Add myself to the new parent cache.
			parent.updateCacheForReturnEvent(this, false);
		}
...
public void removeMeFromLoanCache()
{
	...
	if (getSession().getTransactionInfo().isInCache(LoanImpl.getMetaQuery(), searchReq))
	{
		...
		parent.updateCacheForReturnEvent(this, true);
	}
}

/**	  
* <br>
* method to get the Loan object for this ReturnEvent
* this method currently does not support additional conditional params.
* @return Object : the  parent object Loan for this ReturnEvent.
*/
...
/**	  
* <br>
* method to set the Loan object for this ReturnEvent.
* @param Object : the  parent object Loan for this ReturnEvent.
*/
...
/**	  
* <br>
* method to get the old Loan object for this ReturnEvent
* this method currently does not support additional conditional params.
* @return Object : the  old parent object Loan for this ReturnEvent.
*/
public	LoanImpl	getOldLoan()
{
	...
	/**	  
	* <br>
	* method to get the FKLoan attribute for the ReturnEvent
	...
	*/
	...
	/**	  
	* <br>
	* method to set the FKLoan attribute for the ReturnEvent
	...
	*/
	...
	/**	  
	* <br>
	* method to get the old FKLoan attribute for the ReturnEvent
	...
	*/
	...
	/**	  
	* <br>
	* method to get the TimeStamp attribute for the ReturnEvent
	...
	*/
	...
	/**	  
	* <br>
	* method to set the TimeStamp attribute for the ReturnEvent
	...
	*/
	...
	/**	  
	* <br>
	* method to get the old TimeStamp attribute for the ReturnEvent
...
/**	  
* <br>
* method to get the PKReturnEvent attribute for the ReturnEvent
* @return long : the  value of the attribute PKReturnEvent as long.
*/
public long	getPKReturnEvent() 
{
	return getData("PKReturnEvent").getlong();
}

/**	  
* <br>
* method to set the PKReturnEvent attribute for the ReturnEvent
* @param long : value of the attribute PKReturnEvent as long.
* @return nothing
*/
public void	setPKReturnEvent(long value)
{
	// The code to do convertion from the primitive data
	// to the one which can be stored goes here.
	Data dataVal = getData("PKReturnEvent");
	dataVal.setlong(value);
}

/**	  
* <br>
* method to get the old PKReturnEvent attribute for the ReturnEvent
* @return long : the  value of the old attribute PKReturnEvent as long.
*/
public long	getOldPKReturnEvent()
{
	return getData("PKReturnEvent").getPreviouslong();
}
...
throws ServerException
	...
	if ( aSession.isTransactionInProgress())
		count = aSession.getTransactionInfo().getObjectsCount(ReturnEventBaseImpl.getMetaQuery(), xdac, searchReq);
	else
	{
		con = aSession.getConnection( xdac, true );
  	  	count = xdac.getRowCount( ReturnEventBaseImpl.getMetaQuery(), searchReq, con );
	}
...
{ 
	...
	try {
		VSMetaTable table = ReturnEventBaseImpl.getMetaQuery().getChildMostTable();
		if ( table != null )
...
{ 
	...
	Properties props = ((VSORBSessionImpl)aSession).getMyDataServiceLoginForObject("ReturnEvent");
