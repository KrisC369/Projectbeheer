...
public abstract class ReserveEventBaseImpl extends  DataObject
	
{
	public static Logger logger = Logger.getLogger("LIBs0199831.ReserveEvent");
	
	public ReserveEventBaseImpl () {
		super();
		addListeners();
	}
	...
	*/
	...
	static {
		q= new VSQueryDefinition( "ReserveEvent" );	
		q.setPackageName(deployedFromRepository);
		
		VSMetaColumn c = null;
		VSMetaTable t = new VSMetaTable("ReserveEvent");
		//{{META_QUERY_COLUMN_CTOR
		c = new VSMetaColumn("PKReserveEvent", DataConst.BIGINT);	
		...
		c = new VSMetaColumn("FKReservation", DataConst.BIGINT);	
		c.setAlterability(false);
		...
		// Register a foreign key from  to Reservation.
		t.addForeignKeyColumn( "Foreign Key To Reservation", "FKReservation" );
	...
	//END_META_QUERY_COLUMN_CTOR}}
		q.addTable( t );
		q.populateQCArray();
		VSQueryDefinition qTemp = (VSQueryDefinition)getMetaQuery( "ReserveEvent",deployedFromRepository ); 
		if ( qTemp == null ) {			
