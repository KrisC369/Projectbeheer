...
public abstract class DestroyEventBaseImpl extends  DataObject
	
{
	public static Logger logger = Logger.getLogger("LIBs0199831.DestroyEvent");
	
	public DestroyEventBaseImpl () {
		super();
		addListeners();
	}
	...
	*/
	...
	static {
		q= new VSQueryDefinition( "DestroyEvent" );	
		q.setPackageName(deployedFromRepository);
		
		VSMetaColumn c = null;
		VSMetaTable t = new VSMetaTable("DestroyEvent");
		//{{META_QUERY_COLUMN_CTOR
		c = new VSMetaColumn("PKDestroyEvent", DataConst.BIGINT);	
	...
	//END_META_QUERY_COLUMN_CTOR}}
		q.addTable( t );
		q.populateQCArray();
		VSQueryDefinition qTemp = (VSQueryDefinition)getMetaQuery( "DestroyEvent",deployedFromRepository ); 
		if ( qTemp == null ) {			
