...
abstract class pCopyBase extends versata.pdx.html.VSPagePDX implements java.io.Serializable
{
	...
	public pCopyBase (PLSApp parentApp) throws VSException, java.beans.PropertyVetoException {
		...
		add(T1Copy);
		    				T3Library.Init("T3Library", T1Copy, "Library", "", "", "[PKLibrary] = [?FKLibrary]", "", "PKLibrary, amountOfMembers, amountOfCopies, Name, City, PhoneNumber, WebAddress, LibraryState",getPackageName() + ".genericPick", this);
			T3Library.setParentTableAlias("");
		        
		add(T3Library);
			T4Loan.setName("T4Loan");
			T4Loan.setSession( parentApp.getSession() );
			T4Loan.setQueryInfo("Loan", "", "[FKCopy] = [?PKCopy]", "", true);
			T4Loan.setPage(this);
			T4Loan.setDataDependency(true, false);
		       T4Loan.setDataSource(T1Copy);
		...
		add(T4Loan);
			...
			T5Loan.setQueryInfo("Loan", "", "[FKCopy] = [?PKCopy]", "", true);
			T5Loan.setPage(this);
			T5Loan.setDataDependency(true, false);
		       T5Loan.setDataSource(T1Copy);
		...
		add(T5Loan);
			...
			T9Reservation.setQueryInfo("Reservation", "", "[FKCopy] = [?PKCopy]", "", true);
			T9Reservation.setPage(this);
			T9Reservation.setDataDependency(false, false);
		       T9Reservation.setDataSource(T1Copy);
		...
		add(T9Reservation);
			...
			T10Reservation.setQueryInfo("Reservation", "", "[FKCopy] = [?PKCopy]", "", true);
			T10Reservation.setPage(this);
			T10Reservation.setDataDependency(false, false);
		       T10Reservation.setDataSource(T1Copy);
		...
		T2pLibrary.setSourceDataSource(T1Copy);
		...
		T7pLoan.setTargetPageName(getPackageName() + ".pLoan");
			...
			T7pLoan.setDataDependency(true, false);
		...
		T6pLoan.setTargetPageName(getPackageName() + ".pLoan");
			...
			T6pLoan.setDataDependency(true, false);
		...
		T8pLoan.setSourceDataSource(T1Copy);
		T8pLoan.setTargetPageName(getPackageName() + ".pLoan");
			...
			T8pLoan.setRelnWhere("[FKCopy] = [?PKCopy]");
			T8pLoan.setDevWhere("");
			T8pLoan.setRelnFromParent(true);
			T8pLoan.setOrderBy("");
			T8pLoan.setTargetPageCaption("T8Loan");
			T8pLoan.setDataDependency(true, false);
		...
		T13pReservation.setSourceDataSource(T1Copy);
		T13pReservation.setTargetPageName(getPackageName() + ".pReservation");
			...
			T13pReservation.setRelnWhere("[FKCopy] = [?PKCopy]");
